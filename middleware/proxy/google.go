...
// google is a proxy that forwards everything to dns.google.com using https.
type google struct {
	client   *http.Client
...
func (g google) Protocol() protocol { return googleProto }
...
func (g *google) Exchange(state request.Request) (*dns.Msg, error) {
	...
	buf, err := g.do(addr.Name, v.Encode())
	if err != nil {
...
const (
	googleProto protocol = "https_google"
